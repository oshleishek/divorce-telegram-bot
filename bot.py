...
• Чоловік/дружина згодні → не буде затягувань
...
• Є згода чоловіка/дружини → швидше
...
• Чоловік/дружина проти → потрібна чітка стратегія
...
Не знаєте де чоловік/дружина - це не перешкода для розлучення.
...
✅ Доводимо, що ви намагалися повідомити чоловіка/дружину
✅ Отримуємо рішення суду

<i>Навіть якщо чоловік/дружина пропав/ла безвісти - ви маєте право на нове життя. Ми допоможемо.</i>
""",

    'E2': """
🔍 <b>{segment_name}</b>

Чоловік/дружина проти і може уникати - знаємо як з цим працювати.

💰 <b>Вартість:</b> {cost}
⏱ <b>Строки:</b> {time}

<b>Ваша ситуація:</b>
• Чоловік/дружина проти розлучення
...
<i>Важливо: суд не може відмовити вам у розлученні через те, що чоловік/дружина переховується. Ми це знаємо і використовуємо.</i>
"""
}
...
Наш помічник адвоката <b>зв'яжеться з вами протягом 30 хвилин</b>, щоб узгодити зручний час консультації.
...
# ЗАМІНИ ЦЮ ФУНКЦІЮ ПОВНІСТЮ (РОЗДІЛЕНИЙ ОФФЕР)
async def send_first_offer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Відправляє розділений оффер (Цінність -> Пауза -> Ціна)"""
    
    # Отримуємо дані
    first_name = context.user_data.get('first_name', 'Клієнт')
    segment_name = context.user_data.get('segment_name', 'Ваш випадок')
    # Очищаємо назву сегменту від емодзі для тексту
    cleaned_segment = segment_name.split(' ', 1)[-1]
    
    chat_id = update.effective_chat.id if update.effective_chat else context.user_data.get('telegram_id')

    # === ПОВІДОМЛЕННЯ 1: ЦІННІСТЬ ===
    text_part_1 = f"""
🎁 <b>{first_name}, У МЕНЕ ДЛЯ ВАС СПЕЦІАЛЬНА ПРОПОЗИЦІЯ</b>

Ми пропонуємо не просто «розмову з юристом», а <b>стратегію вирішення вашої проблеми</b>.

<b>Що ви отримаєте (Результат):</b>
1. <b>Персональний план дій:</b> Чітка дорожня карта саме для вашої ситуації ({cleaned_segment}).
2. <b>Захист від помилок:</b> Як гарантовано не втратити майно та права на дітей.
3. <b>Спокій:</b> Ви вийдете з чітким розумінням, що робити завтра.
"""
    ...
    text_part_2 = """
💰 <b>Вартість: всього 990 грн</b> (замість звичайних 2000 грн).
<i>Пропозиція діє до кінця тижня.</i>

🎯 <b>ГОЛОВНЕ (ЗНЯТТЯ РИЗИКУ):</b>
Якщо ви вирішите працювати з нами далі — ці 990 грн <b>ПОВНІСТЮ ЗАРАХОВУЮТЬСЯ</b> в оплату основної послуги!

<i>Тобто консультація для вас — фактично безкоштовна.</i>
"""
...
<i>(Результат: чоловік/ може вивести активи або набрати боргів, які стануть спільними).</i>
