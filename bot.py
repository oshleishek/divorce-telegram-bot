...
# 📝 ПОКРАЩЕНИЙ ОФФЕР (з персоналізацією та реверс-ризиком)
def get_personalized_offer(first_name):
    """Генерує персоналізований оффер"""
    return f"""
🎁 <b>{first_name}, У МЕНЕ ДЛЯ ВАС СПЕЦІАЛЬНА ПРОПОЗИЦІЯ</b>

Зазвичай консультація з адвокатом коштує 2000 грн.

Але для тих, хто пройшов цей тест, ми даємо <b>знижку 50%</b>:
<b>Консультація всього за 990 грн</b> ⏰ <i>(діє 24 години)</i>

<b>Що ви отримаєте на консультації:</b>
✅ Детальний розбір вашої ситуації (60 хвилин)
✅ Покроковий план дій
✅ Відповіді на всі питання
✅ Розрахунок точної вартості послуг
✅ Стратегію як уникнути типових помилок
...
async def send_first_offer(update: Update, context: ContextTypes.DEFAULT_TYPE, first_name):
    ...
    offer_text = get_personalized_offer(first_name)
    
    keyboard = [
        [InlineKeyboardButton("📅 Записатися на консультацію (990 грн)", callback_data='book_consultation')]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
