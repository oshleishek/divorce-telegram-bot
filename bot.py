...
Розрахунок займе <b>2 хвилини</b> і це абсолютно <b>безкоштовно</b>.

<i>Кожна ситуація унікальна. Тому я задам кілька питань, щоб дати точний прогноз саме для вас.</i>

💡 <i>Якщо ви випадково натиснете не туди — просто напишіть /start, щоб почати спочатку.</i>
...
# =====================================================
# ЖИВІ ПРИКЛАДИ (БЕЗ ЗАГОЛОВКІВ, НАТИВНІ)
# =====================================================
...
MINI_CASES = {
    ...
    'yes_children_no_consent': [
        """
<i>Розумію вашу тривогу. Це розповсюджена ситуація.

Нещодавно ми допомагали Марині з Харкова. Її чоловік теж був категорично проти і погрожував забрати сина. Але закон на боці інтересів дитини.

Ми зібрали докази турботи матері, і суд залишив дитину з нею, встановивши чіткий графік зустрічей для батька. Головне — не панікувати і діяти юридично грамотно.</i>
""",
        """
<i>Ситуація непроста, але вирішувана.

У нас був клієнт Олег з Дніпра. Дружина хотіла обмежити його спілкування з сином через образи. Ми допомогли йому зібрати характеристики та докази участі у вихованні.

Суд повністю відхилив вимоги дружини про обмеження. Батько зберіг повноцінний зв'язок з дитиною. Ваші права як батька можна і треба захищати.</i>
"""
    ],
    ...
    'no_children_yes_consent': [
        """
<i>Це чудово, що ви домовилися мирно. У таких випадках наше завдання — зробити так, щоб ви взагалі не витрачали на це час.

Наприклад, Оксана з Києва звернулася з запитом "все під ключ". Ми все оформили самі. Їм з чоловіком навіть не довелося йти до суду — вони просто отримали готове рішення поштою через 2.5 місяці.</i>
""",
        """
<i>Коли є згода, процес можна пройти дуже швидко і комфортно.

Наш клієнт Андрій з Одеси переживав, що загрузне в бюрократії. Ми підготували документи за один день. Його участь зайняла сумарно 1 годину часу, решту ми зробили самі.</i>
"""
    ],
    ...
    'yes_property': [
        """
<i>Розділ майна — це завжди делікатний момент, де важливо врахувати кожну деталь.

Нещодавно ми вели справу, де дружина вклала в квартиру подаровані батьками кошти, а чоловік переживав за свій бізнес.

Ми не просто ділили "50/50", а довели особистий внесок дружини (вона отримала більшу частку житла) і захистили бізнес чоловіка від розділу. Справедливість можлива.</i>
"""
    ],
    ...
    'abroad': [
        """
<i>Відстань зараз не є перешкодою. Багато наших клієнтів розлучаються дистанційно.

Наприклад, Яна виїхала до Німеччини, а чоловік залишився тут. Вона думала, що треба їхати назад для судів. Ми організували все онлайн: вона оформила довіреність в консульстві, і ми отримали розлучення без жодного її візиту в Україну.</i>
""",
        """
<i>Ви можете пройти весь процес, не відриваючись від роботи чи життя за кордоном.

Наш клієнт Максим працює в Польщі. Він не міг приїхати на засідання. Ми зв'язалися через месенджер, підготували документи, і він отримав розлучення, фізично знаходячись у Познані.</i>
"""
    ],
    ...
    'unknown_location': [
        """
<i>Те, що ви не знаєте адреси або чоловік не виходить на зв'язок — не глухий кут.

У нас була схожа ситуація з Оленою із Запоріжжя. Чоловік поїхав і зник. Ми провели процедуру "розлучення за відсутності" через офіційні оголошення. Це зайняло трохи більше часу, але Олена отримала офіційне розлучення без його участі.</i>
""",
        """
<i>Відсутність другого з подружжя не позбавляє вас права на нове життя.

Дмитро з Одеси теж не знав, де зараз дружина. Ми подали позов за її останньою відомою адресою. Суд прийняв рішення про розлучення заочно. Це абсолютно законна процедура.</i>
"""
    ],
    ...
    'default': [
        """
<i>Ми розуміємо ваші побоювання. Найчастіший страх наших клієнтів: "Я зроблю помилку, і це коштуватиме мені грошей або нервів".

Наша перша задача — дати вам <b>чіткий, покроковий план дій.</b> Коли ви бачите шлях і розумієте свої права, 90% стресу зникає миттєво.</i>
"""
    ]
}
...
async def question_4(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Q4: Місце супруга (З ІСТОРІЄЮ, АЛЕ БЕЗ ПАУЗИ-АНОНСУ)"""
    query = update.callback_query
    await query.answer()
    ...
    # Отримуємо релевантний текст (кейс/приклад)
    mini_case = get_mini_case(context.user_data)
    
    # 1. Відправляємо мікро-комміт (редагуємо старе повідомлення)
    await query.edit_message_text(
        microcommit,
        parse_mode='HTML'
    )
    
    # 2. Невелика пауза для природності (ніби бот друкує)
    await asyncio.sleep(1.5)
    await context.bot.send_chat_action(chat_id=chat_id, action=ChatAction.TYPING)
    
    # 3. Відправляємо історію/приклад (просто текст, без заголовків)
    ...
    # 4. Пауза, щоб дати час прочитати (3 секунди достатньо для короткого тексту)
    await asyncio.sleep(3)
    await context.bot.send_chat_action(chat_id=chat_id, action=ChatAction.TYPING)

    # ... (код отправки кейса и пауз выше остается тем же) ...
